...
// Local adaptations for this repository's split root-plus-/demo/ layout:
// 1. Internal links resolve against the linking page's URL, not the origin,
//    because generated pages use page-relative hrefs; a trailing /index.html
//    is normalized to the directory route before inbound-link matching.
// 2. Sitemap entries listed in config.nonHtmlSitemapPaths (the assistant
//    guide, an unresolved sitemap-policy decision) are checked for existence
//    only; HTML page assertions and the orphan check do not apply to them.
'use strict';
...
for (const entry of entries) {
    ...
    if (url.origin !== origin.origin || url.href !== entry.loc) report('defect', `${entry.source}: noncanonical location ${entry.loc}`);
    if (sitemapUrls.has(entry.loc)) report('defect', `${entry.source}: duplicate location ${entry.loc}`);
    ...
    if ((config.nonHtmlSitemapPaths || []).includes(url.pathname)) continue;
    const html = fs.readFileSync(file, 'utf8');
    pages.set(entry.loc, { html, rel, pathname: url.pathname });
    const canonical = first(html, /<link\b[^>]*rel=["']canonical["'][^>]*href=["']([^"']+)["']/i)
        || first(html, /<link\b[^>]*href=["']([^"']+)["'][^>]*rel=["']canonical["']/i);
    if (canonical !== entry.loc) report('defect', `${rel}: canonical ${canonical || '(missing)'} does not equal ${entry.loc}`);
...
function internal(href, base) {
    if (!href || /^(mailto:|tel:|data:|javascript:)/i.test(href)) return null;
    let url;
    try { url = new URL(decode(href), base || origin); } catch { return null; }
...
const nonHtml = new Set((config.nonHtmlSitemapPaths || []).map(p => new URL(p, origin).href));
for (const [url, sources] of inbound) {
    if (nonHtml.has(url)) continue;
